...
GameGuardian 是一款能够 **实时捕捉用户情绪变化** 的应用程序，专为提升游戏体验而设计。它可在后台运行，通过 **前置摄像头**捕捉您的表情，推测您的情绪状态，并结合 **当前游戏场景**，提供 **智能、个性化的交互**。我们的目标是帮助玩家 **减少游戏中的负面情绪**，优化整体游戏体验，并避免长时间游戏可能带来的身体与心理负担。
...
3. **系统设置**
   - **电池管理**：设置中，电池->后台耗电管理->GameGuardian允许后台高耗电。（可选）
...
## 🧭使用指南
1. 打开app进入首页，可选择不同游戏类型。完成设置后，点击“确认设置”。当选择“其他游戏”，会默认先进入竖屏模式。如果游戏的视图方向和当前页面不同，请点击“rorate view”来选转屏幕方向，以达到和游戏方向一致。

2. 进入控制页面，第一次会请求相机权限、存储权限、屏幕捕捉权限同意即可。在玩家游戏开始前，点击“开始追踪”，选择同意录制的屏幕，然后同意其请求通知权限。

3. 开始追踪用户情绪后，用户切换到游戏画面，进入对局。如果检测到玩家出现情绪波动，该软件会返回安慰、鼓励到话语，以达到干预玩家负面情绪、缓解玩家烦躁、愤怒的目的。
...
## 🔒 隐私与数据安全
- **本地存储**：少量情绪识别数据和游戏截屏数据，以及加速度计的数据。
- **上传云端**：仅在情绪波动时 **触发 5 秒录屏截图**，用于分析游戏情境，屏幕中的文字信息将会被**模糊处理**，再 **上传至服务器** 以改善干预策略。
